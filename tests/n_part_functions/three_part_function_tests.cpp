...
/***************************************************** 
 *
 * Test suite for the ThreePartFunction class 
 *
 *****************************************************/
...
bool check_the_values()
{
	std::string file_solution("tpf_results.txt");
	...
	// Test 1 - no offset
	double offset = 0.0;
	const std::vector<std::vector<double>> points = {{0.0, 50.0}, {49.4949, 74.7475},
													 {73.7374, 74.7475}, {100.0, 73.0003}};
	std::vector<double> temp(2,0.0);
	ThreePartFunction tpf(points, offset);	
	...
	print_solution(computed_values, "tpf_computed_no_offset.txt");		

	// Test 2 - offset (same results as no offset)
	offset = 340.0;
	ThreePartFunction tpf_offset(points, offset);	
	std::vector<std::vector<double>> computed_values_offset;
	for (const auto& xy_exp : expected_values) {
		// Compute
		y = tpf_offset(xy_exp.at(0)+offset);
		// Save for printing
	...
	offset = -40.0;
	ThreePartFunction tpf_neg_offset(points, offset);	
	std::vector<std::vector<double>> computed_values_neg_offset;
	for (const auto& xy_exp : expected_values) {
		// Compute
		y = tpf_neg_offset(xy_exp.at(0)+offset);
		// Save for printing
		temp.at(0) = xy_exp.at(0) + offset;
		temp.at(1) = y;
	...
	// Optionally, print the computed values
	print_solution(computed_values_neg_offset, "tpf_computed_with_neg_offset.txt");		
...
bool past_the_limit()
{
	std::string file_solution("tpf_results_past_limits.txt");
	double y = 0.0;
	double offset = 340.0;
	const std::vector<std::vector<double>> points = {{0.0, 50.0}, {49.4949, 74.7475},
													 {73.7374, 74.7475}, {100.0, 73.0003}};
	...
	// Optionally, print the computed values
	print_solution(computed_values, "tpf_computed_past_limit.txt");		
...
bool one_value()
{
	...
	const double val = 1.57;
	double y = 0;
	ThreePartFunction tpf(val);
