...
using dist_range_sampling = double (Infection::*)(const double, const double);
...
using age_rates_caller = bool (Infection::*)(int, double, double, double, bool);
using age_rates_caller_no_bool = bool (Infection::*)(int);
using age_rates_caller_double = bool (Infection::*)(int, double);
...
bool check_age_dependent_rates(age_rates_setter, age_rates_getter, age_rates_caller_no_bool,
								Infection&,	std::map<std::string, double>, 
								std::map<std::string, std::tuple<int, int, double>>);
bool check_age_dependent_rates(age_rates_setter, age_rates_getter, age_rates_caller_double,
								Infection&,	std::map<std::string, double>, 
								std::map<std::string, std::tuple<int, int, double>>,
...
bool check_distribution(dist_range_sampling, const double, const double, Infection&, double);
...
bool infection_transmission_test()
{
	...
	if (!check_distribution(&Infection::get_uniform, 0.0, 20.0, infection, 10.0)){
		std::cerr << "Error in sampling a range in the uniform distribution" << std::endl;
		return false;		
	}
...
bool check_mortality_rates(Infection& infection)
{
	...
	// Vector of vaccination corrections: correction for probability of 
	// being asymptomatic, probability of being hospitalized, and mortality
	std::vector<double> corrections{2.0, 0.7, 0.1};
	...
	return check_age_dependent_rates(&Infection::set_mortality_rates, &Infection::get_mortality_rates,
					&Infection::will_die_non_icu, infection, 
					raw_mortality_rates, mortality_rates, corrections);
}
...
bool check_hospitalization_rates(Infection& infection)
{
	...
	// Correction for vaccinated agents
	const double correction = 0.25;
	...
	return check_age_dependent_rates(&Infection::set_hospitalized_fractions, 
									 ...
									 infection, raw_rates, hsp_rates, correction);
}
...
bool check_exposed_never_sy(Infection& infection)
{
	...
	// Increased probability after vaccination
...
bool check_non_icu_mortality(Infection& infection)
{
	...
	// Vector of vaccination corrections: correction for probability of 
	// being asymptomatic, probability of being hospitalized, and mortality
	std::vector<double> corrections{1.0, 1.0, 1.0};
	...
	for (const auto& rate : exp_rates){
		...
		for (int i=0; i<n_tot; ++i){
			if (infection.will_die_non_icu(age, corrections.at(0), corrections.at(1), 
					corrections.at(2)) == true){
				n_affected++;
			}	
		}
...
// This test is currently fit only for a special case of mortality with 
// no probability of hospitalizations and death in ICU
								...
								std::vector<double> corrections)
{
	...
	for (const auto& rate : created_rates){
		...
		for (int i=0; i<n_tot; ++i){
			if ((infection.*call)(age, corrections.at(0), corrections.at(1), 
					corrections.at(2), false) == true){
				n_affected++;
			}	
		}
		double fr_affected = (static_cast<double>(n_affected))/(static_cast<double>(n_tot));
		double fr_exp = std::get<2>(rate.second)*corrections.at(2);
...
/// \brief Function for checking different types of age-dependent rates
bool check_age_dependent_rates(age_rates_setter set_rates, age_rates_getter get_rates,
								age_rates_caller_double call, Infection& infection, 
								std::map<std::string, double> raw_rates, 
								std::map<std::string, std::tuple<int, int, double>> expected_rates,
								const double correction)
{
	...
	for (const auto& rate : created_rates){
		...
		int age = std::get<0>(rate.second)+1;
		for (int i=0; i<n_tot; ++i){
			if ((infection.*call)(age, correction) == true)
				n_affected++;	
		}
		double fr_affected = (static_cast<double>(n_affected))/(static_cast<double>(n_tot));
		double fr_exp = std::min(1.0, std::get<2>(rate.second)*correction);
		if (!float_equality<double>(fr_affected, fr_exp, 0.01)){
			std::cerr << "Age-dependent rates: computed rate for age " << age 
					  << " not equal nominal" << std::endl;
			return false;
...
/// \brief Test sampling of an infection distribution on an interval against expected mean
bool check_distribution(dist_range_sampling dist, const double low, const double high, Infection& infection, double exp_mean)
{
	std::vector<double> rnum;
	for (int i=0; i<1000000; ++i)
		rnum.push_back((infection.*dist)(low, high));
	// Compare mean value
	double rng_mean = std::accumulate(rnum.begin(), rnum.end(), 0.0)/(static_cast<double>(rnum.size()));
